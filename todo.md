...
- [ ] **Confirm PostHog is actually reporting after the first promote (session 51).** The tag is gated to `CF_PAGES_BRANCH === "main"`, so it cannot be tested anywhere before production — that was the accepted cost of keeping staging out of the dataset. After the first `/ayso promote`, check PostHog for live events from www.ayso13.org, confirm `/temp` reports as `/temp` and not `/temp.html`, and check the `csp-report` Worker KV for any `us.i.posthog.com` or `us-assets.i.posthog.com` violations. A CSP miss is the most likely failure and would be silent on the page.
