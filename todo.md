...
- [ ] **Add a `/resources/air-quality-policy/` page** — sister to `/resources/heat-policy/` and `/resources/rain-policy/`. Region 13 had AQI guidance on the old WordPress site; review the WP XML export at `~/Downloads/aysoregion13.WordPress.2026-05-01\ \(1\).xml` for prior policy text. Standard youth-sports thresholds: AQI ≤ 100 normal, 101–150 sensitive groups limit exertion, 151–200 modify activity, 201+ cancel outdoor activity. Cross-link from `/resources/safety/`, `/resources/weather/`, footer, sidebar nav. Add live AQI reading + closure recommendation to `/resources/weather/` (depends on next item).
- [ ] **Air-quality API integration in `workers/weather-api/`** — add live AQI data to the `/api/weather` envelope so the conditions pages (`/resources/weather/` + `/temp`) can render current AQI alongside temperature, WBGT, and rainfall. Candidate data sources, in order of preference: (a) **AirNow.gov** (EPA, free with API key, official data, ~hourly cadence) — best authority for school-sports decisions; (b) **PurpleAir** (free, dense PWS-like network of citizen sensors, sub-hourly) — closer to our fields but less authoritative; (c) **OpenWeatherMap Air Pollution API** (free tier, global coverage). Implementation: add `airQuality` block to the envelope (`aqi`, `category`, `dominantPollutant`, `closureRecommended`, `reason`); compute closure flag based on threshold from the new air-quality-policy page; cache in KV like rain.
