...
- [x] **Redirect cap diagnosed** — `/audit`-driven investigation found Cloudflare Pages was silently capping `site/src/_redirects` at rule #224, dropping ~344 of our 568 rules in production. Symptoms: high-traffic legacy URLs (`/heat/`, `/victory/`, `/pictures/`, `/safety/`, `/weather/`, `/Schedule`, `/4u-playground`, `/index.php`, plus tail of WP-uploads PDFs) 404'd on prod despite being in `_redirects` on `main`. Sanitizing non-ASCII chars in comments didn't help — empirical cap at exactly rule 224.
- [x] **_redirects reordered by traffic priority** — Tier 1 (top ~224 rules, evidence-driven via CF Analytics 404s + GSC reports) put first; Tier 2 (~346 rules, defensive/long-tail) below a clear marker comment. No rules deleted. Restored all currently-broken paths to working state immediately. Added `/dashboard`, `/dashboard/`, `/register/volunteer/`, `/register/volunteer`, promoted `/start/` to Tier 1; demoted `/aysou`, `/play`, `/policy`, `/lc`, `/refund` to Tier 2.
- [x] **Redirect Worker built** — new `workers/redirects/` Cloudflare Worker handles all 570 redirects with no cap. `scripts/generate-map.js` parses `site/src/_redirects` and emits `src/map.js` (562 exact + 8 splat). Worker exact-match-first (O(1)), then splat patterns, then `fetch(request)` passthrough to Pages. Routes: `www.ayso13.org/*` and `staging.ayso13.org/*`; the more-specific `/api/weather` route still wins for the weather Worker. **All 570 rules verified working via live HEAD checks** (`scripts/test-live.mjs`, 2.5s parallel run).
- [x] **Auto-deploy on _redirects edits** — new `.github/workflows/deploy-redirects-worker.yml` runs `npm run deploy` (preflight + build + tests + wrangler) on any push to main that touches `site/src/_redirects` or `workers/redirects/**`. Same chain as local.
- [x] **direnv setup for Cloudflare account ID** — `.envrc.example` committed as template; `.envrc` gitignored. `npm run` scripts in both Worker projects (`workers/redirects/`, `workers/weather-api/`) run a `preflight` script that errors out if `CLOUDFLARE_ACCOUNT_ID` is unset — prevents wrangler from deploying to the wrong account on a machine with multiple Cloudflare accounts.
- [x] **GitHub secrets renamed for clarity** — `CF_API_TOKEN` → `CLOUDFLARE_PAGES_API_TOKEN`, `CF_ACCOUNT_ID` → `CLOUDFLARE_ACCOUNT_ID` (deduplicated with the existing Worker-deploy secret of the same name). Three workflows + `wait-for-cf-deploy.sh` updated. Tokens stay separate (Pages:Read vs Workers:Edit) per least-privilege; only the naming was unified.
