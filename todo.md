...
- [ ] **Post 2023–present board minutes** — In session 17, the 2014–2022 archive (79 PDFs) was moved to `archive/board-minutes/` (out of served path), and `/about/board-minutes/` was rewritten to direct visitors to `rc@ayso13.org` for archive requests. **Still missing:** 2023, 2024, 2025 minutes — none are publicly posted. Drive folder: `~/Library/CloudStorage/GoogleDrive-matthew@ayso13.org/Shared drives/Region 13 Operations/Board & Governance/Agenda & Minutes 2025-2026/`. Most are `.gdoc` (require manual File→Download→PDF) or `.docx` (convert via `soffice --headless --convert-to pdf *.docx` once LibreOffice is installed). Once converted, drop into `site/src/assets/docs/minutes/` (recreate the dir) and update `/about/board-minutes/` with a current-season list.
