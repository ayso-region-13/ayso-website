...
- [ ] **Fix UTC off-by-one in date filter + footer transform** — `site/.eleventy.js` formats commit dates with `d.toISOString().split("T")[0]` which is UTC. Any commit made after ~5 PM Pacific renders as the next day in `sitemap.xml` `<lastmod>` and in the per-page "Last updated" footer when built on Cloudflare Pages (which runs UTC). Currently 2 sitemap URLs show dates that haven't happened yet, and ~14 pages dated "today" were actually committed late yesterday Pacific. Fix: replace the `date` filter and the `[DATE]` transform's `iso` line with `new Intl.DateTimeFormat("en-CA", { timeZone: "America/Los_Angeles", year: "numeric", month: "2-digit", day: "2-digit" }).format(d)` (en-CA outputs YYYY-MM-DD natively). 5-line change. Practical impact is minor — Google/Bing accept off-by-one lastmod fine, footer is cosmetic — but it's a real bug surfaced when verifying sitemap accuracy.
