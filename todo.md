...
- [ ] **Confirm PostHog events are landing in the UI (session 51).** The page-side half is verified: promote `33435643614` succeeded, `www.ayso13.org/` and `/temp` both serve `posthog.init` with all six options intact, `/temp` carries its `capture_pageview: false` plus the pinned manual `$pageview`, production CSP serves both PostHog hosts, `array.js` returns HTTP 200, and the token resolves on **US** cloud (`us-assets` 200 / `eu-assets` 404, so the region assumption was right). What is left needs account access: confirm real events appear in PostHog for www.ayso13.org, that `/temp` shows as `/temp` and not `/temp.html`, and that the `csp-report` Worker KV logged no `*.posthog.com` violations. A CSP miss would be silent on the page, so check the KV even if events look fine.
