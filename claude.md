...
- `/logo/` — Logo assets
- `/workers/weather-api/` — Cloudflare Worker that powers `/resources/weather/`. Cron-polls Tempest station 33318 + NWS forecast every 5 min, computes WBGT, tracks rolling rainfall in KV, serves at `www.ayso13.org/api/weather` (also bound on `staging.ayso13.org/api/weather`). **Slack notifications (live 2026-06-13):** after each cron tick, three notifiers post to `#notify-weather` (`C0BB7JJ0XRN`) via `SLACK_BOT_TOKEN` — (1) field-closure threshold trips (`closureRecommended` false↔true: heat L5 / rain / AQI), (2) new/ended NWS active alerts (dedup by id), (3) rain-forecast heads-up (PoP ≥ `POP_FORECAST_THRESHOLD`, default 60, throttled 1/24h per period). Notifier state lives in KV `notify:closure` / `notify:nwsAlerts` / `notify:rainForecast`; pure decision logic is unit-tested (`npm test`). Notifications fire only on the cron path (never fetch cold-start) and run under `Promise.allSettled` so they can't break the feed. **Deploy:** this Worker binds KV (`WEATHER_KV`), so deploying it needs a token with **Workers KV Storage: Edit** — the shared `CLOUDFLARE_API_TOKEN` (in `.envrc` and the CI repo secret) only has **Workers Scripts: Edit + Account Settings: Read**, which deploys the no-KV workers (redirects, field-maps) but trips `code 10023` here (and on csp-report, the other KV-bound Worker). Two ways to deploy: (a) **add Workers KV Storage: Edit** to that token in the CF dashboard → then `npm run deploy` works like the other workers; or (b) until then, deploy via OAuth: `cd workers/weather-api && env -u CLOUDFLARE_API_TOKEN npx wrangler login` then `env -u CLOUDFLARE_API_TOKEN npm run deploy`. Secrets: `TEMPEST_TOKEN`, `AIRNOW_API_KEY`, `SLACK_BOT_TOKEN` (not in git); station ID, lat/lon, channel id, PoP threshold, and KV id are in `wrangler.toml`. See `workers/weather-api/README.md`.
