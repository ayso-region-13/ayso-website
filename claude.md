...
- **SEO + analytics**: GA4 `G-9YM9ZDW1J9`, GSC `sc-domain:ayso13.org`, GA4 property `307558725`. Per-project Google creds in `.seo-creds/` (gitignored, direnv-symlinked to `~/.config/claude-seo`). Query commands + creds gotchas → **`docs/seo-data-pulls.md`**. The tag is the **`gtag/js`** snippet, not a Tag Manager `gtm.js` container, and there is no GTM container anywhere in this repo — so the Google Ads "update your gtag installation before October 02, 2026" notice (received 2026-08-27) **does not apply**; verified against source and eight live pages, don't re-investigate. **The snippet now lives in one place**, `_includes/ga4.njk`, included by `base.njk`, `temp.njk`, and the newsletter layout (`_includes/newsletter.njk`) — added 2026-09-24 so a third copy for newsletters didn't mean a third place to drift. `temp.njk` sets `gaPinnedPath = "/temp"` before including it, which pins `page_location` to `window.location.origin + '/temp'` because its permalink is `/temp.html` but its public URL is `/temp`; the newsletter layout needs no pin since its permalink already matches its public URL.
...
- **Newsletter archive** (added 2026-09-24): `.github/workflows/sync-newsletters.yml` runs daily at 6am PT, pages through EmailOctopus's `/campaigns` API, and snapshots every sent Soccer News newsletter (list `5ee4e68d-…`) into `site/newsletters/`: one cleaned `<slug>.html` per email plus an `index.json`. We snapshot instead of linking out because the API has no signed web-version URL: the real link carries an `s=` signature (64 hex characters) we can't compute, confirmed 2026-09-24 by hitting a real link with and without it (200 vs. 400). Retention is the current and previous Pacific calendar year, floored at 2026-01-01 (the request was "all of 2026," and the old hand-pasted 2023–2025 links were retired with it). The sync writes to **both** `staging` and `main` with `PROMOTE_TOKEN`, the same pattern as Slack field status: a `GITHUB_TOKEN` push starts no workflows, so nothing would deploy. `rebuild-production.yml` now also fires on `site/newsletters/**`, so a new newsletter reaches www the same day without a promote. Each newsletter renders on its own minimal layout (`_includes/newsletter.njk`, not `base.njk`) so Tailwind's preflight can't collide with the email's own table-based CSS; `resources/newsletters.md` swaps a `[NEWSLETTER ARCHIVE]` placeholder for `_includes/newsletter-archive.njk`, same pattern as `[SPONSOR TIERS]`. **A leftover `{{…}}` or `{%…%}` merge tag fails the sync on purpose** (`cleanHtml` in `site/scripts/lib/newsletters.mjs`): the cleaner handles the three EmailOctopus layouts seen in real data, and a fourth one showing up is meant to break the build loudly rather than publish a literal tag; the fix is a new targeted rule plus a fixture test, never a blanket allowlist. Current snapshot: 14 newsletters, 2026-02-07 to 2026-09-24. Not yet merged into `staging` or pushed. **Pending rollout**: merge, add the `EMAILOCTOPUS_API_KEY` GitHub secret, push, promote once so `main` has the templates and widened `rebuild-production.yml` filter, then run the workflow by hand.
  - Titles strip only the subject's **leading** emoji ("⚽ Week 2 News" → "Week 2 News," but "FIVE DAYS until ⚽!" keeps its ball). The sync pages through **every** `/campaigns` result rather than stopping at the cutoff, since EmailOctopus's sort order isn't documented and a draft can sit mid-list.
  - The workflow's `main` steps run on every successful sync, so a `main` push that fails one day self-heals on the next daily run. It isn't on `deploy-watchdog.yml`'s list; a run that never gets a runner is silent, and the next day's run is the recovery.
  - Files: `site/scripts/sync-newsletters.mjs` (the runner) + `site/scripts/lib/newsletters.mjs` (pure helpers: retention, slugging, HTML cleaning). Tests: `cd site && npm test` (`node --test`, matches `scripts/**/*.test.mjs`). The data loader's test sits at `site/scripts/newsletters-data.test.mjs` rather than next to `_data/newsletters.js`, because **Eleventy 3 loads every `.mjs` file under `_data/` as global data**: a test file there would itself become a data source.
  - `site/newsletters/` is **generated, never hand-edit**, same rule as `workers/redirects/src/map.js`.
...
├── newsletters/           ← GENERATED snapshot from EmailOctopus — never hand-edit.
│                             One <slug>.html per newsletter + index.json.
└── scripts/
    ...
    ├── sync-newsletters.mjs  ← Daily EmailOctopus → site/newsletters/ sync
    └── lib/newsletters.mjs   ← Pure helpers behind the sync (retention, slugs, HTML cleaning)
```
...
- **Slack field status and announcements reach production on their own** (fixed 2026-09-06). The bot writes both files to `staging` *and* `main` (`commitToBothBranches`), a design from April 2026 when CF's Git integration still watched `main`. The 2026-06-18 move to CI Direct Upload disabled that and nothing replaced it, so for roughly three months the write to `main` was a no-op for prod: a rain closure went live on staging and sat invisible on www until someone ran `/ayso promote` by hand (last occurrence: 2026-09-06, 11 minutes stale). `rebuild-production.yml` now also triggers on `push` to `main` limited to `site/src/_data/fieldstatus.json` + `announcements.json` — narrow on purpose, so ordinary content still reaches www only through a deliberate promote. Its job `if:` skips merge commits, because a promote already deploys prod itself and would otherwise get a redundant deploy queued behind it. **Being a push trigger, it runs the copy of the workflow on `main`, so it is inert until promoted.** Verified end to end 2026-09-06: run `34049461658`, event `push`, all 14 steps green, www served the new message with no promote.
  - **Re-submitting the same values does nothing at all, and that is not a bug.** The Contents API creates a commit even when the content is identical, so `/ayso field` with unchanged status *and* message produces an **empty commit** — same tree as its parent. An empty commit changes no files, matches no `paths:` filter, and therefore starts no workflow: no staging deploy, no prod rebuild, nothing in `#notify-website-status`. It looks exactly like a broken trigger. This cost real debugging time on 2026-09-06 (commits `a80adb8` / `69b46b0`, both empty). **To test the deploy path, change the text**, don't retype it.
  - The deploy watchdog covers this workflow as of 2026-09-06. A rebuild evicted from the shared `deploy-pages-prod` group gets its own verdict, `silent-superseded-rebuild`, because the recovery differs from a dropped promote's: the message says a field status or announcement is live on staging and `main` but not on www. It is **not** auto-retried — the staging deploy is the only thing the watchdog re-dispatches, since a prod deploy stays a human decision.
...
- `sponsors.js` — sponsor logos and URLs (not currently displayed)
- `sponsorTiers.json` — tier cards on `/volunteers/sponsors/`, CMS-editable as "Sponsorship Tiers". The page itself is plain CMS markdown (`volunteers/sponsors.md`); `page.njk` swaps a `[SPONSOR TIERS]` paragraph for `_includes/sponsor-tiers.njk`, and `llms-full.njk` does the same swap so the tiers reach `/llms-full.txt`. Delete the placeholder line and the cards disappear. A plain InLeague link whose text starts with "Donate" becomes a button (same transform as the Register button)
...
- `newsletters.js` — Reads the `site/newsletters/` snapshot (see the Newsletter archive headline-state bullet) and, for each one, splits its `<head>` `<style>` blocks from its `<body>` markup so `_includes/newsletter.njk` can host the email in a page of its own. Email `<img>` tags are tagged `eleventy:ignore` so the sitewide image transform skips them — those images stay on EmailOctopus's CDN. An empty or missing `site/newsletters/` directory returns an empty list rather than failing the build.
...
- **Coach App / Referee Scheduler buttons** (added 2026-09-18) — `site.json` `portals` (keyed by section: `coaches` → coach.ayso13.org, `referees` → referee.ayso13.org) is the one source of truth. It drives three things: a sidebar card (`_includes/portal-card.njk`, pulled into **both** `page.njk` and `ask-the-referee.njk` sidebars, renders only on sections with a `portals` entry), the footer Contact-column links, and the `portal-buttons` transform in `.eleventy.js`. That transform turns a portal link **alone in its own paragraph** into a `.btn-portal` (burgundy + coral strip); a portal link inside a sentence stays a plain link. So in CMS markdown, put `[Open the Coach App](https://coach.ayso13.org/)` on a line by itself to get a button. Team managers deliberately get no portal. `site.json` `buttonLinks` (added 2026-09-24, currently just `https://stats.ayso13.org/submit`, the referee game card submission) gets the same standalone-paragraph button treatment without a sidebar card or footer link.
...
---
*Last updated: 2026-09-24 — daily newsletter archive sync from EmailOctopus; GA4 snippet consolidated into one shared include; referees submit game cards online at stats.ayso13.org/submit (tent no longer collects them); new `site.json` `buttonLinks` for standalone-link buttons; feedback/incident links added for coaches and families.*
