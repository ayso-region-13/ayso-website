...
- **`field-maps` branch** — field-map editor tool. New `workers/field-maps/` Worker serves an Access-gated SPA at `fields.ayso13.org` where board members draw field layouts over Mapbox satellite (parametric: rotatable field rectangles w/ auto HOME/AWAY sidelines by compass; practice **grids** subdivided into labeled cells; lines, text words, markers — goal/restroom/parking/host-tent/picture-day). Export is deterministic (Mapbox **Static Images API** base + 2D-canvas annotation composite, never a GL screenshot — `sharp` can't run in the Workers runtime). On save the Worker commits the PNG (`site/src/images/fields/<slug>-<variant>.png`) + re-editable annotation JSON (`site/src/_data/fieldmaps/<slug>.json`) to `staging` in one **atomic Git Data API commit** (ref→blobs→tree→commit→ref). Site render: Eleventy namespaces `_data/fieldmaps/` as the `fieldmaps` global; `page.njk` has a `section=="fields"` block rendering `fieldmaps[page.fileSlug].variants` as `<img>` (eleventy-img → WebP). Pure geometry in `public/geo.js` is Node-tested. **Not yet wired**: Mapbox URL-restricted token, CF Access app (`ACCESS_TEAM_DOMAIN`/`ACCESS_AUD` vars), fine-grained `GITHUB_TOKEN` secret, `fields.ayso13.org` DNS — see `workers/field-maps/README.md`. On-map drag interactions (move/resize/rotate handles) are written but need a live test once the token is set. Migration: per field, delete old hand-placed markdown maps as new ones land. Resume: `git switch field-maps`.
