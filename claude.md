...
---
*Last updated: 2026-07-09 (session 38 — debounced the weather closure Slack notice (heat hysteresis 89.7→88.0 + 15-min dwell, Slack-only) to stop boundary flapping; ran the out-of-area GA4 re-check (durable `site/scripts/ga4-outofarea.py`); verified the recurring CSP FB/DoubleClick blocks aren't from our source. Session 37 — expanded `/llms.txt` + build-generated `/llms-full.txt`, out-of-area registrant routing. Full session-by-session changelog in `claude-history.md`.)*
