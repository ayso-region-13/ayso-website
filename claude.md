...
- **Field maps — read from the platform at build time** (migrated 2026-07-22/24, session 40; replaced the retired `workers/field-maps/` editor Worker). The map editor now lives in the **ayso-platform** admin console (`~/dev/ayso-platform`), which serves `fields.ayso13.org` and its staging twin `fields-staging.ayso13.org`. This repo no longer stores maps: `site/src/_data/fieldmaps.js` → `_data/lib/fetchFieldMaps.js` fetches `GET /public/fields` (venue index) + `GET /public/fields/<slug>` (detail, plus the `overview` slug which the index omits by design) at build time, cached by `eleventy-fetch` for 1 day so a platform outage serves last-good instead of failing the build. The committed `_data/fieldmaps/*.json` and the 40 `images/fields/*-<variant>.png` exports are **deleted** (the 21 field `.jpg` photos stayed).
  ...
  - **A missing map asset hard-fails the whole site build** (hit 2026-08-06). The platform's D1 advertises a variant whose PNG is absent from the R2 `UPLOADS` bucket → `/field-images/<slug>-<variant>.png` 404s → **eleventy-img returns `undefined` rather than throwing** → `metadata.png` in `optimizeVariantImage` throws `Cannot read properties of undefined (reading 'png')` and 11ty dies at the global-data stage. Every staging deploy fails, including unrelated CMS content edits. Diagnose by sweeping both instances (`/public/fields` → each `/public/fields/<slug>` → HEAD each `image_url`) — that pinpoints slug/variant immediately. On 2026-08-06 three PNGs were missing from `region13-uploads-staging` only (`allendale-game`, `cornishon-practice`, `fis-upper-practice`) while prod was complete 39/39, which is why promote/rebuild-production kept working (they read `fields.ayso13.org`) and only staging broke. Fix was to copy the prod objects into the staging bucket. **The canonical `ayso13-worker-deploy` token has neither R2 nor D1 permission** (`Authentication error [code: 10000]`), so R2/D1 work needs the dashboard, a scoped token, or the interactive `wrangler login` OAuth session — and note `wrangler r2 object` / `d1 execute` default to **local** storage, where a miss reads exactly like a real absence. Root cause of the disappearance was never established; the one lead is that the three affected `field_maps` rows carry millisecond-identical `updated_at` values across both D1 instances, which looks like a row copy that didn't move the R2 objects (unconfirmed — see `claude-history.md`).
...
---
*Last updated: 2026-08-12 (session 48 — the Rose City RollOut hero artwork was re-exported at **2560×996** (same 2.57:1 composition, a straight 2×), and the banner went **full-bleed** at the user's direction: the old `max-w-screen-xl` cap existed only because a 1399px source would have upscaled past 1280, so with 2560 available the cap moved to `max-w-[2560px]` and the section now spans the viewport like the rotation hero. Height follows width since the artwork is never cropped: 498px @1280 (unchanged), 747px @1920, 996px @2560. Because the artwork width now tracks the viewport all the way up, the overlay's padding, type and accent strip are all `vw`-based with `min()` ceilings computed at 2560px, so the box holds one proportion at every width and freezes precisely where the image does; at 1280 it renders identically to session 46. **The load-bearing find: the `<img>` was emitting a single-candidate srcset.** In `@11ty/eleventy-img` 6.0.4 a plain valid `width` attribute overrides `eleventy:widths` and is read as "generate exactly this one size" (`src/image-attrs-to-posthtml-node.js` lines 76-82), so `width="1399"` had been shipping the full-size file to phones for the whole of session 46-47 — and session 46's note that a 2560 drop-in "needs no code change" was wrong for the same reason: it would have quietly served a 237 KB 2560px JPEG to a 390px phone. Removing `width`/`height` (eleventy-img emits its own intrinsic pair from the largest candidate, so CLS stays covered) and adding `eleventy:widths="600,1000,1400,1920,2560"` restored a real ladder — a 390px viewport now pulls 20 KB of WebP instead of 100 KB+. Verified with headless Chromium at 390/768/1280/1920/2560: geometry and chosen candidate both as predicted. Detail in `claude-history.md`.) (session 47 — new `/referees/abuse/` page for the AYSO / Area 1C **Referee Abuse Prevention Program (RAPP)**, enforced across Area 1C from Fall 2026. Four PDFs imported from the AYSO national CDN and renamed into `site/src/assets/docs/` (`respecting-our-referees-2026`, `area-1c-rapp-implementation-protocols-2026`, `area-1c-rapp-flow-chart-2026`, `ussf-referee-abuse-prevention-policy-531-9`). Two gotchas worth keeping: **RAPP abuse and ordinary dissent are separate tracks, not one escalation** — Area 1C limits RAPP to "extreme and deliberate" behavior and leaves normal dissent to warnings/Cautions/Send-Offs, and the page keeps them as distinct sections; and the 2022 `respect-the-referee.pdf` is **not** superseded by the near-identically-titled "Respecting Our Referees" (2026) — the old one is a four-page Region 13 policy (zero tolerance, coach accountability, Law 5 facts-of-the-game), the new one is a two-page AYSO/USSF standards summary, so both are listed with dates. Contact routes through `referee@ayso13.org` rather than a named RRA because the RAPP roles are appointed annually. Cross-linked from the referee sidebar, `/referees/`, `/referees/resources/`, `/contact/feedback/`, `/about/policies/`, `/coaches/`, `/families/pledge/`, `/families/support/`, and `llms.txt`. Redirects Worker 648 → 652 rules (`/rapp`, `/referee-abuse`, ± slash) — **needs its own deploy**. Spec: `docs/superpowers/specs/2026-08-11-referee-abuse-page-design.md`. Detail in `claude-history.md`.) (session 46 — temporary Rose City RollOut event banner replaces the 5-photo home hero until Aug 29, behind the `homeHero.js` `mode` switch; the original hero and its rotation script are preserved verbatim in the else branch, so reverting is one word. Announcement bar disabled. Three CSS gotchas recorded in `claude-history.md`: `clamp()` floors break proportional overlays, percentage padding counts as zero inside a shrink-to-fit absolutely-positioned box, and **Eleventy `--incremental` does not re-copy passthrough CSS — if a Tailwind class change seems to have no effect in local dev, diff `_site/assets/css/style.css` against `src/assets/css/style.css` before touching markup.** Open item, closed in session 48: the 1399px source was re-exported at 2560×996 — and "no code change needed to adopt it" was wrong on both counts, see session 48.) (session 45 — surfaced the clubhouse request Typeform: a bullet in Coaching Resources on `/coaches/`, a "Using the Clubhouse" section on `/volunteers/`, a `/clubhouse` slug (redirects Worker now 648 rules), and an `llms.txt` entry. Deliberately NOT on `/register/forms/` — that page is "Required Forms" and sits in the Families sidebar. Gotcha recorded: `workers/redirects/src/map.js` is generated from `site/src/_redirects`, so the first pass hand-edited the map, passed local tests, and had both rules erased by `generate-map.js` during deploy. Promoted and verified on www.ayso13.org the same day; that promote also shipped session 44's stranded backlog. Detail in `claude-history.md`.) (session 44 — diagnosed the staging deploy failures: three missing field-map PNGs in the `region13-uploads-staging` R2 bucket made eleventy-img return `undefined`, so `metadata.png` threw and every staging build died, stranding 7 CMS commits; prod was complete, which is why manual promotes kept working. Restored the objects, added the **deploy watchdog** for runs that report nothing at all (runner-allocation failures, superseded promotes), added **`/ayso staging`** for a manual rebuild, and narrowed the staging debounce to pushes only. Then, prompted by "Allendale has no game field": an **ungenerated** map (`image_url: null`) no longer kills the build — it renders no section — while an **advertised-but-missing** one still fails loudly, and the spurious `allendale/game` variant was deleted from both platform D1 instances plus its R2 objects (it was an empty save in the current editor on 2026-08-05, *not* legacy import data). Security review also found and fixed two fail-open bugs: the watchdog could suppress its own alert, and the Slack bot's authorization allowed everyone whenever the allowlist lookup failed. Detail in `claude-history.md`.) (session 43 — "Marshall Fundamental" renamed to "Thurgood Marshall Secondary School": field page title/description came via CMS, and this session swept what the CMS can't reach — the fields sidebar label in `navigation.js` (now the short "Marshall"), the two Winter Stars location lines, the Grad Series "Marshall Field" line, and a "Thursgood" typo in `add-descriptions.js`. Note the **field-map `alt` text still reads "Marshall Fundamental"** — it comes from the platform's `ayso_fields.title` + the per-variant stored `alt`, so that fix is ayso-platform D1 work (prod + staging), not this repo. Detail in `claude-history.md`. Session 42 — surfaced the Withdrawal Form: `/register/forms/` was orphaned from `navigation.js` and is now in the Register + Families sidebars, `/families/faqs/` gained a withdrawal/refund Q&A, the `/register/` H2 became "Refunds and Withdrawals" (anchor `#refunds-and-withdrawals`), `/refund` was repointed and `/withdraw`+`/withdrawal` slugs added (redirects Worker now 646 rules — needs its own deploy), `/resources/documents/` bullets collapsed, and `llms.txt` gained Required Forms + Refunds entries. Detail in `claude-history.md`. Session 41 — WBGT is now read from the Tempest station's own `wet_bulb_globe_temperature` field instead of derived locally. The old `computeWbgt`/`approxGlobeTemp` clamped its globe-temperature term to a constant +25°C, and that clamp bound above ~86 W/m² of solar at calm wind, i.e. every daylight hour: measured solar and wind had NO effect on the published value, which reduced to the shade formula plus a flat +9°F. Against the station's own figure across 20 samples it ran +2.0 to +8.5°F too hot, repeatedly reporting CIF Level 5 "outdoor activity suspended" where the station read Level 2-3. Verified post-deploy: near-identical inputs, 86.6°F/L3 → 79.2°F/L1. `computeWbgt`, `stullWetBulb`, `approxGlobeTemp` deleted; `fetchTempest` throws if the field is absent because `cifLevel(null)` returns Level 1 and would announce an all-clear with the sensor down. Also repointed the NWS forecast to Victory Park (4dp — api.weather.gov 301s on finer precision) and removed a false claim from `/resources/weather/` that WBGT came from the Bernard 1999 approximation within ~1°F of ISO 7243. tomorrow.io evaluated and rejected (free plan 403s both `solarGHI` and `wetBulbGlobeTemperature`). Investigation + dataset: `docs/superpowers/specs/2026-07-24-wbgt-source-design.md`. Session 40 — field maps moved to the ayso-platform admin console: this site now fetches them from the platform's public API at build time (`_data/lib/fetchFieldMaps.js`) and optimizes the images locally; the `field-maps` Worker, the committed `_data/fieldmaps/*.json`, and the 40 committed map PNGs are all deleted. Follow-up cleanup on 07-24 removed `field-maps` from `deploy-workers.yml` (it was failing the workflow on main) and completed the Phase 3 cutover so prod builds read `fields.ayso13.org` instead of the staging instance. Session 39 — fixed the closure Slack notice posting a generic "threshold reached" bullet instead of naming heat: `closureReasons` now uses the notifier's hysteresis-latched `heatActive` flag (not the raw live CIF level) and includes the live WBGT °F value, so a closure that dipped into the 88–89.7°F deadband during the dwell still reads e.g. "Heat: WBGT 89.1°F — outdoor activity should be suspended". Session 38 — debounced the weather closure Slack notice (heat hysteresis 89.7→88.0 + 15-min dwell, Slack-only) to stop boundary flapping; out-of-area GA4 re-check; verified recurring CSP FB/DoubleClick blocks aren't ours. Full session-by-session changelog in `claude-history.md`.)*
