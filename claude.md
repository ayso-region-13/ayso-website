...
---
*Last updated: 2026-06-25 (session 37 — expanded `/llms.txt` (Resources & Safety, Tournaments) and added a build-generated `/llms-full.txt` full-text export; both now route out-of-area registrants to AYSO's national Region Locator. Session 36 — disabled the noisy NWS active-alert Slack notifier by default; added a front-end °F/°C toggle to /resources/weather/, /temp, /resources/heat-policy/. Full session-by-session changelog in `claude-history.md`.)*
