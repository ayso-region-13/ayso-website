# AYSO Region 13 — Completed Backlog Archive

Split out of `todo.md` on 2026-08-12 (session 48) to keep the active list cheap to read. Every completed item is preserved verbatim; nothing here is actionable. Narrative context for the same work lives in `claude-history.md`.
...
- [x] **Delete the spurious Allendale `game` map — DONE 2026-08-06.** Allendale Park has no game field, and the variant was an empty save in the current editor (`elements_json = "[]"`, 2026-08-05T15:52Z), not legacy import data. Row deleted from both prod and staging D1 plus both orphaned R2 objects; both instances verified at 0 missing of 38 variants and `/fields/allendale/` now shows only the practices map. Record + what's still open (why the objects went missing; the API not checking `png_ref` resolves; nothing stopping another empty variant): `~/dev/ayso-platform/docs/handoff-allendale-game-map.md`.
...
- [x] **Out-of-area flow re-check — DONE 2026-07-05** (via `site/scripts/ga4-outofarea.py`, a durable replacement for the old `/tmp` scripts; run locally, verify `ga4_property_id` 307558725 first per `feedback_check_seo_creds_first`). Findings, last 28 days:
  - **`region_locator_click`: 1 event / 1 user.** The event fires correctly but click-through to AYSO's national Region Locator is negligible.
  - **Home `/` bounce:** CA 24.5% (n=728) vs non-CA **40.1%** (n=187) — down sharply from the 66.2% baseline (windows differ, 28d vs 14d, so directional). The banner appears to be working: out-of-area visitors bounce less.
  - **`/register` bounce:** CA 58.0% (n=398) vs non-CA 69.9% (n=73) — high across the board (inflated by the off-site InLeague CTA); small/noisy non-CA sample.
  - **`/register/find-your-region/`: 23 views / 20 users** (~steady vs ~22/28d baseline).
  - **Read:** the funnel is thin — few out-of-area visitors reach find-your-region, and almost none click the locator link. The home banner reduces non-CA bounce, but the locator CTA barely converts.
...
- [x] **Clubhouse request Typeform linked from the site** — `https://ayso13.typeform.com/to/xCIGB2TA` now reachable from `/coaches/` (a bullet in Coaching Resources) and `/volunteers/` (a short "Using the Clubhouse" section), plus a `/clubhouse` + `/clubhouse/` slug (map now 639 exact + 9 splat = 648) and an `llms.txt` entry under Volunteers pointing at the slug. Promoted and verified on prod the same day.
- [x] **Deliberately not added to `/register/forms/`** — that page is titled "Required Forms" and renders in both the Register and Families sidebars; a volunteer facility request is neither required nor a family concern. Note this compounds the session-23 inventory finding that the reimbursement form has no canonical home. Regrouping that page into Family / Volunteer sections remains open, unscheduled.
- [x] **Gotcha: `workers/redirects/src/map.js` is generated** — the rules went in there first, local `npm test` passed at 639 rules because the tests read the same file, and `generate-map.js` then erased both during the deploy's build step. Staging deployed green with `/clubhouse` 404ing. Source of truth is `site/src/_redirects`. Warning added to CLAUDE.md.

## Completed ✓
### Session 40 (2026-07-21 → 07-24) — field maps moved to the platform; editor Worker retired

- [x] **Site reads field maps from the ayso-platform public API at build time** (PRs #4, #5, #6 — all merged + promoted 2026-07-23). `_data/lib/fetchFieldMaps.js` fetches the venue index + per-venue detail (+ the `overview` slug), `eleventy-fetch`-cached 1 day so a platform outage serves last-good. Images optimized locally at fetch time (webp+png 600/1200/2000w) so both the inline `<picture>` and the lightbox `<a href>` are local — the raw ~4.5 MB cross-origin PNG is gone from every page. CI needs `User-Agent: ayso13-fieldmaps-build` plus a matching Cloudflare "Skip" rule, else CF bot protection 403s the runner.
- [x] **`workers/field-maps/` deleted** — the editor lives in ayso-platform now (`fields.ayso13.org` is served from there); the Worker was already removed from Cloudflare. Also deleted the 40 orphaned committed map PNGs and the `_data/fieldmaps/*.json`; kept all 21 field `.jpg` photos. Jefferson redirects repointed to the platform's slug-keyed `/field-images/jefferson-practice.png` (survives map edits, unlike a hashed `/img/` filename).
- [x] **Fixed `Deploy Workers` failing on main** — the promote that deleted `workers/field-maps/` matched that path filter, so CI tried `cd workers/field-maps` and exited 1 (run 30050867405). Removed field-maps from the path filter, dispatch choices, and both deploy loops.
- [x] **Phase 3 cutover completed (2026-07-24)** — `FIELDS_API_BASE` had never been set in CI, so both staging *and* prod builds read `fields-staging.ayso13.org`. Verified both platform instances served identical data (23 venues, matching variants), then wired promote → `https://fields.ayso13.org`, staging → `https://fields-staging.ayso13.org`, and flipped the in-code default to prod so an unset env reads published maps.
- [x] **Stale branches deleted** — `fields-staging`, `fields-ci-ua-skip`, `fields-cleanup`, and the original May `field-maps` branch (its two unmerged commits only touched the now-deleted `workers/field-maps/wrangler.toml`).
...
### Session 48 (2026-08-12) — RollOut hero at 2×; docs cleanup

- [x] **Deploy the `redirects` Worker for the withdrawal slugs (session 42) — DONE 2026-08-12.** Shipped with the session-48 promote, which advanced the worker to 652 rules. Verified live on prod: `/withdraw`, `/withdrawal` and `/refund` all 301 → `/register/#refunds-and-withdrawals`, and the session-47 `/rapp` + `/referee-abuse` slugs 301 → `/referees/abuse/`. Note `deploy-redirects-worker.yml` is branch-scoped and fires automatically on a push to `main`, so a promote deploys it — the standing note that redirects "need their own deploy" means they don't ship from a *Pages* deploy, not that the step is manual.
