...
> **⚠️ This endpoint returns the OLDEST reports, not the newest.** Keys sort
> ascending by ISO timestamp, and the handler reverses only the single page it
> fetched, so once the namespace exceeds `MAX_LIST_LIMIT` (1000) you get the
> oldest 1000 and nothing recent. On 2026-08-31 the namespace held 9,899 keys
> and this endpoint was serving four-week-old data while looking authoritative.
> Until it paginates on the `list()` cursor (logged in `todo.md`), read recent
> reports with wrangler instead:
>
> ```bash
> npx wrangler kv key list --namespace-id=d6a804159968475db6bcbf7354b45d0f \
>   --prefix="$(date -u +%Y-%m-%d)" --remote
> ```
>
> `--remote` is not optional. Without it you read an empty local store, and a
> local miss looks exactly like "no violations".
...
Known-benign violations are dropped before the KV write. The list and the
evidence for each entry live in `src/ignore.js`; `npm test` covers it.

Currently one host: **`stats.g.doubleclick.net`**. It is generated by our own
GA4 tag, not a third party. Verified 2026-08-31 across 985 reports — every one
carried `tid=G-9YM9ZDW1J9` across 754 distinct `cid`s and ordinary browsers.
The trigger is `npa=0` (ad personalization on). The same URLs carry `ngs=1`,
and the GA4 Admin API reports `googleSignalsSettings = GOOGLE_SIGNALS_DISABLED`,
so Google Signals is **not** the cause, despite what earlier notes claimed.

Why drop rather than allowlist: blocking is the correct outcome, so the report
carries nothing we act on — but at ~500/day it buried everything else. A check
for PostHog violations that day came back "clean" only because the doubleclick
volume had pushed all other reports out of reach.

Dropped reports are still logged, so `npm run tail` shows them live:

```
CSP violation (ignored): https://stats.g.doubleclick.net/g/collect?...
```

`connect.facebook.net` is deliberately **not** ignored. It was assumed to be
extension injection, but that was never verified the way doubleclick now has
been. The bar for this list is verified-benign with the evidence written down.
